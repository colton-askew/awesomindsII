var modeState = {
  ...
  create: function(){
    ...
    //animate text to the bottom; needed in case this state was skipped to before animation finished in pregame
    game.add.tween(text.sprite).to({x: Math.floor(((game.width/1)*(1) -game.width/1)+(game.width/25))}, 250, Phaser.Easing.Default, true);
  ...
  update: function(){
    ...
    text.x = Math.floor(text.sprite.right + game.global.borderFrameSize);
    text.y = Math.floor(text.sprite.centerY + (11*dpr));
